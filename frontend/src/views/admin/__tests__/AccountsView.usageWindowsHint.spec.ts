...
vi.mock('@/api/admin', () => ({
  ...
  getAccountPatrolSettings: vi.fn().mockResolvedValue({ enabled: true, interval_minutes: 30 }),
      delete: vi.fn(),
    ...
    proxies: {
      // 【定制】AccountsView 用 getAllWithCount（裸 getAll 不返回 latency_ms/quality_*/country，
      // 代理选择器就没法展示/排序延迟）。上游 mock 只有 getAll，这里补上。
