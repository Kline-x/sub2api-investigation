  ...
  <div class="mb-4 flex items-center justify-between rounded-lg bg-primary-50 p-3 dark:bg-primary-900/20">
    ...
    <div class="flex gap-2">
      <template v-if="selectedIds.length > 0">
        ...
        <button class="btn btn-secondary btn-sm" @click="$emit('test')">{{ t('admin.accounts.bulkActions.test') }}</button>`r`n        <button class="btn btn-secondary btn-sm" @click="$emit('refresh-token')">{{ t('admin.accounts.bulkActions.refreshToken') }}</button>
...
defineEmits(['delete', 'edit-selected', 'edit-filtered', 'clear', 'select-page', 'select-filtered', 'toggle-schedulable', 'reset-status', 'refresh-token', 'test'])
const { t } = useI18n()
</script>
