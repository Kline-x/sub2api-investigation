...
改 `testGrokAccountConnection`(`account_test_service.go:692`)非 200 分支:

- **429**:保持现有限流逻辑,不置 error
- **其他 4xx**(400/401/403 等):`accountRepo.SetError(ctx, account.ID, errMsg)` 后再返回错误
- **5xx / 网络错误 / 代理故障**:不改状态,仅返回错误
...
| 场景 | 429 | 其他 4xx | 5xx/网络 |
|---|---|---|---|
| 测试(所有入口) | 限流态,不置错 | SetError | 不改状态 |
| 刷新令牌(单个/批量/导入后) | 不置错 | SetError | 不改状态 |
...
- grok 测试 4xx 置错、429 不置错、5xx 不置错(扩展 `account_test_service_grok_test.go`)
