...
- grok 测试模型:传空 modelID,由 `grokDefaultResponsesModel`(= `grok-4.5`)兜底,不硬编码新常量
...
- Produces:
  ...
  - `POST /api/v1/admin/accounts/batch-test`,请求 `{"account_ids":[...]}`,响应 `{total, success, failed, results:[{id,name,status,error_message,latency_ms}]}`——Task 9 前端消费
...
// 定制:后端并发批测,复用 RunTestBackground;grok 账号传空 model 落默认 grok-4.5,
...
func (h *AccountHandler) BatchTest(c *gin.Context) {
	...
	for _, account := range accounts {
		...
		g.Go(func() error {
			...
			testResult, testErr := h.accountTester.RunTestBackground(gctx, acc.ID, "")
			switch {
			case testErr != nil:
				item.Status = "failed"
...
### Task 9: 前端批量测试按钮 + API + 验证 grok-4.5 默认
 ...
 * 失败账号会被自动置为错误状态。
 */
export async function batchTest(accountIds: number[]): Promise<BatchTestResult> {
  const { data } = await apiClient.post<BatchTestResult>('/admin/accounts/batch-test', {
