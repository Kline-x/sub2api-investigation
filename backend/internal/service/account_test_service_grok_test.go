...
// 定制:测试路径「非 429 直接置错」——400 带 free-usage 文案的响应不再走
// 上游的 10 分钟探测冷却,而是直接 SetError(上游断言的是临时不可调度策略)。
func TestObserveGrokTestResponseBodyOnlyQuotaErrorSetsError(t *testing.T) {
	...
	require.Contains(t, repo.lastErrorMsg, "Grok Responses API returned 400")
	body, err := io.ReadAll(resp.Body)
	require.NoError(t, err)
	require.Contains(t, string(body), "free-usage-exhausted")
}

// 定制:测试路径对内容策略 403 同样「非 429 直接置错」(上游这里跳过状态变更)。
func TestObserveGrokTestResponseContentPolicy403SetsError(t *testing.T) {
	...
	require.Equal(t, 1, repo.setErrorCalls)
	require.Contains(t, repo.lastErrorMsg, "Grok Responses API returned 403")
}

// 定制:403 权益拒绝直接 SetError(上游断言的是 30 分钟临时不可调度冷却)。
func TestObserveGrokTestResponseEntitlement403SetsError(t *testing.T) {
