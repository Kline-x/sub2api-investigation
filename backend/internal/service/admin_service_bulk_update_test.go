...
// 【定制】账号批量改到期时间的三态语义（set / clear / omit）——保留，勿删。
func TestAdminServiceBulkUpdateAccountsExpirationTriState(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			require.Equal(t, tt.wantSet, repo.lastBulkUpdate.ExpiresAtSet)
			require.Equal(t, tt.wantValue, repo.lastBulkUpdate.ExpiresAt)
		})
	}
}
